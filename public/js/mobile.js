...
async function loadMobileSkin(skinId) {
  if (!skinId || skinId === 'standard') { SKIN = { id:'standard', hexImages:{}, robberImage:null, buildingImages:{}, roadImages:{} }; return; }
  ...
  try {
    ...
    renderBoardCanvas();
  } catch(e) { SKIN = { id:'standard', hexImages:{}, robberImage:null, buildingImages:{}, roadImages:{} }; }
}
...
function connectWS() {
  ...
  ws.onclose   = () => { setLoading(t('ws_reconnecting')); setTimeout(connectWS,1500); };
...
// ── Init ──────────────────────────────────────────────────────────
async function init() {
  if (!MY_TOKEN) { setLoading(t('err_token_missing')||'Token mancante.'); return; }
...
function onMessage(data) {
  ...
  if (prevSpecials && state) {
    for (const p of state.players) {
      ...
      if (p.hasLongestRoad && !prev.hasLongestRoad)
        showMobToast(`🛤 ${escHtml(p.name)} — Longest Road!`, 4000, 'toast-special');
      if (p.hasLargestArmy && !prev.hasLargestArmy)
        showMobToast(`⚔️ ${escHtml(p.name)} — Largest Army!`, 4000, 'toast-special');
    }
  }
  ...
  if (newDrawn && (!prevLastDrawn || prevLastDrawn.card !== newDrawn.card ||
    ...
    if (newDrawn.playerId === MY_PLAYER_ID) {
      ...
      showMobToast(`🃏 ${cardName}`, 3000, 'toast-dev');
    }
  }

  // Detect trade resolution: I was the proposer, pendingTrade just cleared
  if (prevPendingTrade && !state.pendingTrade &&
...
function renderWaitScreen(curIdx) {
  ...
  const pMedalHTML = [
    p.hasLongestRoad ? '<span class="mob-medal road-medal">🛤🥇</span>' : '',
    p.hasLargestArmy ? '<span class="mob-medal army-medal">⚔️🥇</span>' : '',
  ...
  if (me) {
    document.getElementById('mob-resources').innerHTML =
      RES_LIST.map(r=>`<div class="mob-res-pill"><span>${RES_EMOJI[r]}</span><span class="n">${me.resources[r]||0}</span></div>`).join('');
  }
...
function updateMyResources() {
  ...
  const pills = RES_LIST.map(r=>{
    ...
    return `<div class="mob-res-pill"><span>${RES_EMOJI[r]}</span><span class="n">${n}</span></div>`;
  }).join('');
  // Medal badges
  const medals = [
    me.hasLongestRoad ? '<span class="mob-medal road-medal">🛤🥇</span>' : '',
    me.hasLargestArmy ? '<span class="mob-medal army-medal">⚔️🥇</span>' : '',
...
function renderActionPanel() {
  ...
  expandSheet(); // restore if we were collapsed for robber
...
function showSetupPanel() {
  ...
  if (autoMode) {
    const label = autoMode === 'road_initial'
      ? (t('mob_build_label_road') || 'Tap an edge for the road')
      : (t('mob_build_label_settlement') || 'Tap a vertex for the settlement');
  ...
  if (!discardPanelBuilt) {
    ...
    document.getElementById('mob-discard-resources').innerHTML = RES_LIST.map(r=>`
      <div class="mob-discard-row">
        <label>${RES_EMOJI[r]} (${me.resources[r]||0})</label>
...
function updateBuildButtons() {
  ...
  const ca=state.diceRolled&&!state.pendingRobber;
  ...
  setBB('mob-btn-devcard',    ca&&res.sheep>=1&&res.wheat>=1&&res.ore>=1&&state.devDeckSize>0, ca);
...
function setMobBuildMode(mode) {
  ...
  const banner=document.getElementById('mob-build-banner');
  const labels={
    road:                 t('mob_build_label_road')        || 'Tap an edge for the road',
    settlement:           t('mob_build_label_settlement')   || 'Tap a vertex for the settlement',
    city:                 t('mob_build_label_city')         || 'Tap your settlement to upgrade',
    robber:               t('mob_build_label_robber')       || 'Tap a hex for the robber',
    road_initial:         t('mob_build_label_road')         || 'Tap an edge for the road',
    settlement_initial:   t('mob_build_label_settlement')   || 'Tap a vertex for the settlement',
  };
  if(mode){
...
document.getElementById('mob-btn-road').addEventListener('click',()=>{
  ...
  if (isSetup && state?.waitingForRoad) {
    ...
    const banner=document.getElementById('mob-build-banner');
    banner.textContent=t('mob_build_label_road')||'Tap an edge for the road';
    banner.classList.remove('hidden');
    renderBoardCanvas();
...
document.getElementById('mob-btn-settlement').addEventListener('click',()=>{
  ...
  if (isSetup && !state?.waitingForRoad && !state?.pendingSetupEndTurn) {
    ...
    const banner=document.getElementById('mob-build-banner');
    banner.textContent=t('mob_build_label_settlement')||'Tap a vertex for the settlement';
    banner.classList.remove('hidden');
    renderBoardCanvas();
...
function drawBoardHexes() {
  ...
  for (const hex of state.board.hexes) {
    ...
    if (!skinImg) {
      ...
      bctx.fillText(RES_EMOJI[hex.resource]||'?',bpx(hex.cx),bpy(hex.cy)-hs*.26);
    }
    // Number token
    if (hex.number) {
...
function drawBoardPorts() {
  ...
  for (const port of state.board.ports) {
    ...
    bctx.fillText(isG?'🌀':RES_EMOJI[port.type],bx,by-r*.22);
...
document.getElementById('mob-btn-bank').addEventListener('click',()=>{
  ...
  document.getElementById('mob-trade-give').innerHTML=RES_LIST.map(r=>{
    ...
    return `<button class="mob-res-btn" data-res="${r}" onclick="selBankGive('${r}')" ${dis}>
      <span class="re">${RES_EMOJI[r]}</span><span>${rn}</span><small>${have}/${ratio}</small>
    </button>`;
  }).join('');
  document.getElementById('mob-trade-receive').innerHTML=RES_LIST.map(r=>{
    ...
    return `<button class="mob-res-btn" data-res="${r}" onclick="selBankRecv('${r}')">
      <span class="re">${RES_EMOJI[r]}</span><span>${rn}</span>
    </button>`;
  }).join('');
...
window.selBankGive=r=>{
  ...
  const me=state.players[MY_PLAYER_ID];
  document.getElementById('mob-trade-ratio').textContent=t('pt_ratio_info',getMobTradeRatio(me,r),me.resources[r]||0,RES_EMOJI[r]||r);
};
...
function renderPTModal(){
  ...
  document.getElementById('mob-pt-rows').innerHTML=RES_LIST.map(r=>{
    ...
    return `<div class="mob-pt-row">
      <span class="mob-pt-emoji">${RES_EMOJI[r]}</span>
      <div class="mob-pt-name">${(t('res')||{})[r]||r}</div>
...
function showTradeAccept(trade){
  ...
  const fmt=obj=>Object.entries(obj).filter(([,a])=>a>0).map(([r,a])=>`${a}×${RES_EMOJI[r]}`).join(' ');
...
window.playMobDev=type=>{
  ...
  if(type==='yearOfPlenty'){
    ...
    document.getElementById('mob-yop-res').innerHTML=RES_LIST.map(r=>{
      const rn=(t('res')||{})[r]||r;
      return `<button class="mob-res-btn" data-res="${r}" onclick="togYOP('${r}')"><span class="re">${RES_EMOJI[r]}</span><span>${rn}</span></button>`;
    }).join('');
...
function showMobGains(total){
  ...
  if(me && resEl){
    resEl.innerHTML = RES_LIST.map(r=>{
      ...
      return `<div class="mob-res-pill${g?' gained':''}"><span>${RES_EMOJI[r]}</span><span class="n">${n}</span>${badge}</div>`;
    }).join('');
  }
  ...
  const lines = Object.entries(gains).map(([pid,g])=>{
    ...
    const res=Object.entries(g).map(([r,n])=>`${n}×${RES_EMOJI[r]}`).join(' ');
