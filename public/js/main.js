...
function DEV_NAMES_MAP() { return {
  knight:       t('devname_knight')       || '⚔️ Knight',
  victoryPoint: t('devname_vp')           || '⭐ Victory Point',
  roadBuilding: t('devname_road_build')   || '🛤 Road Building',
  yearOfPlenty: t('devname_yop')          || '🌻 Year of Plenty',
  monopoly:     t('devname_monopoly')     || '👑 Monopoly',
}; }
...
function badgeHTML(p) {
  const badges = [];
  if (p.hasLongestRoad)  badges.push('<span class="medal-badge road-medal"  title="Longest Road">🛤🥇</span>');
  if (p.hasLargestArmy)  badges.push('<span class="medal-badge army-medal"  title="Largest Army">⚔️🥇</span>');
  const kn = p.knightsPlayed || 0;
  if (kn > 0 && !p.hasLargestArmy) badges.push(`<span class="knight-count">⚔️×${kn}</span>`);
  else if (kn > 0 && p.hasLargestArmy) badges[badges.length-1] = `<span class="medal-badge army-medal" title="Largest Army">⚔️🥇<sup>${kn}</sup></span>`;
  return badges.join('');
}
...
function onMessage(data) { if (window._onMessageHook) window._onMessageHook(data);
  ...
  if (newDrawn && (!prevLastDrawn || prevLastDrawn.card !== newDrawn.card ||
      prevLastDrawn.playerId !== newDrawn.playerId)) {
    showDevCardDrawnPopup(newDrawn);
  }

  // Detect Longest Road / Largest Army badge changes
  if (prevSpecials && state) {
    for (const p of state.players) {
      const prev = prevSpecials[p.id];
      if (!prev) continue;
      if (p.hasLongestRoad && !prev.hasLongestRoad)
        showGameToast(`🛤 ${escHtml(p.name)} — ${t('log_longest_road', p.name, state.longestRoadLength||'5+')}`, 'toast-special', 4000);
      if (p.hasLargestArmy && !prev.hasLargestArmy)
        showGameToast(`⚔️ ${escHtml(p.name)} — ${t('log_largest_army', p.name)}`, 'toast-special', 4000);
    }
  }
...
function getDrawerInsets() {
  ...
  const hudH   = parseInt(style.getPropertyValue('--hud-h'))   || 62;

  // In spectator mode: players panel is always open, no other drawers
  ...
  for (const hex of state.board.hexes) {
    ...
    if (!skinImg) {
      ...
      if (window.__TV_MODE) {
        ...
        ctx.fillText(RES_EMOJI[hex.resource]||'?', px(hex.cx), py(hex.cy) - HEX_SIZE*.26);
      }
    }
...
function drawPorts() {
  ...
  for (const port of state.board.ports) {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(isGeneric ? '🌀' : RES_EMOJI[port.type], bx, by - r * .25);
...
function renderPlayers() {
  ...
  for (const p of state.players) {
    ...
    const resHtml = ['wood','brick','sheep','wheat','ore'].map(r=>
      `<div class="res-badge"><span class="res-icon">${RES_EMOJI[r]}</span><span>${res[r]||0}</span></div>`
    ).join('');
...
function renderHUD() {
  ...
  const phase = (state.phase==='setup1'||state.phase==='setup2')
    ? (state.waitingForRoad ? t('phase_place_road') : t('phase_place_sett'))
  ...
  if (state.phase==='setup1'||state.phase==='setup2') {
    ...
    setupB.textContent = state.waitingForRoad
      ? `${player.name}: ${t('phase_place_road')}`
      : `${player.name}: ${t('phase_place_sett')}`;
  } else setupB.classList.add('hidden');
  robberB.classList.toggle('hidden', !state.pendingRobber);
  if (state.pendingRobber) robberB.textContent = t('banner_robber');
  ...
  if (state.pendingRoadBuilding > 0 && !state.pendingRobber &&
    ...
    setupB2.textContent = `🛤 ${t('btn_road')} — ${state.pendingRoadBuilding} ${state.pendingRoadBuilding===1?t('phase_place_road'):t('sec_build')}`;
  }
}
...
function translateLogEntry(e) {
  ...
  const p = e.params || {};
  const DEV_DISPLAY = {
    knight: t('tab_actions') ? '⚔️' : '⚔️',
    victoryPoint: '⭐',
    roadBuilding: '🛤',
    yearOfPlenty: '🌻',
    monopoly: '👑'
  };
  const rn = r => (t('res') || {})[r] || r; // resource name translated
  switch (e.key) {
    ...
    case 'log_longest_road': return t('log_longest_road', p.name, p.len);
    case 'log_largest_army': return t('log_largest_army', p.name);
...
function openTradeBankModal() {
  ...
  document.getElementById('trade-receive').innerHTML = res.map(r => {
    ...
    return `<button class="res-pick-btn" data-res="${r}" onclick="selectTradeReceive('${r}')">
      <span class="res-emoji">${RES_EMOJI[r]}</span><span>${rName}</span>
    </button>`;
  }).join('');
...
function updateTradeRatioInfo() {
  ...
  document.getElementById('trade-ratio-info').textContent=`Tasso: ${ratio}:1 — Hai ${p.resources[tradeGive]||0} ${RES_EMOJI[tradeGive]}`;
}
  ...
  document.getElementById('discard-resources').innerHTML=
    ['wood','brick','sheep','wheat','ore'].map(r=>`
      <div class="discard-row">
        <label>${RES_EMOJI[r]} ${t('res')[r]||r} (${p.resources[r]||0})</label>
        <div class="stepper">
...
function openDevCardModal() {
  ...
  if (!cards.length) { list.innerHTML=`<p style="color:#c8b080">${t('no_cards')}</p>`; }
    ...
    list.innerHTML=Object.entries(counts).map(([k,cnt])=>{
      ...
      else if (isKnight && !rolled)  badge=`<span class="new-badge before-roll-badge">⚔️ ora!</span>`;
...
function openYOPModal() {
  yopChoices=[];
  const res=['wood','brick','sheep','wheat','ore'];
  document.getElementById('yop-resources').innerHTML=res.map(r=>`<button class="res-pick-btn" data-res="${r}" onclick="toggleYOP('${r}')"><span class="res-emoji">${RES_EMOJI[r]}</span><span>${t('res')[r]||r}</span></button>`).join('');
  openModal('modal-yop');
}
...
function openMonopolyModal() {
  const res=['wood','brick','sheep','wheat','ore'];
  document.getElementById('monopoly-resources').innerHTML=res.map(r=>`<button class="res-pick-btn" onclick="playMonopoly('${r}')"><span class="res-emoji">${RES_EMOJI[r]}</span><span>${t('res')[r]||r}</span></button>`).join('');
  ...
  const rows = RES_LIST.map(r => {
    ...
    <div class="pt-row">
      ...
      <div class="pt-res-center">
        <span class="pt-emoji">${RES_EMOJI[r]}</span>
        <span class="pt-resname">${t('res')[r]||r}</span>
      </div>
...
window.sendTradeOffer = targetId => {
  ...
  for (const [r,a] of Object.entries(offer)) {
    if ((me.resources[r]||0) < a) return showTradeError('Non hai abbastanza ' + RES_EMOJI[r] + ' ' + r);
  }
    ...
    if (anyGains) {
      ...
      for (const card of cards) {
        ...
        pop.innerHTML = Object.entries(gains[pid])
          .map(([r,a]) => `<span>+${a} ${RES_EMOJI[r]}</span>`)
          .join('');
  ...
  setTimeout(() => {
    ...
    for (const card of cards) {
      ...
      pop.innerHTML = Object.entries(deltas[pid])
        .map(([r,d]) => `<span class="${d>0?'delta-pos':'delta-neg'}">${d>0?'+':''}${d} ${RES_EMOJI[r]}</span>`)
        .join('');
      popupContainer.appendChild(pop);
      requestAnimationFrame(() => pop.classList.add('visible'));
    }
...
function renderPlayersWithTradeDeltas(deltas) {
  ...
  for (const p of state.players) {
    ...
    const resHtml = ['wood','brick','sheep','wheat','ore'].map(r => {
      ...
      return `<div class="res-badge${cls}"><span class="res-icon">${RES_EMOJI[r]}</span><span>${res[r]||0}</span>${badge}</div>`;
    }).join('');
...
async function loadSkinAssets(skinId) {
  if (skinId === 'standard') { SKIN = { id:'standard', hexImages:{}, robberImage:null, buildingImages:{}, roadImages:{} }; if (state) { renderBoard(); render(); } return; }
  try {
    const res = await fetch(`/skins/${skinId}/skin.json`);
    if (!res.ok) { SKIN = { id:'standard', hexImages:{}, robberImage:null, buildingImages:{}, roadImages:{} }; return; }
    ...
    SKIN = { id: skinId, hexImages, robberImage, buildingImages, roadImages };
    if (state) renderBoard(); // re-render with new textures (only if state exists)
    ...
    SKIN = { id:'standard', hexImages:{}, robberImage:null, buildingImages:{}, roadImages:{} };
  }
}
...
const DEV_CARD_DESC = {
  knight:        () => t('devcard_knight_desc')  || 'Move the robber and steal a resource',
  victoryPoint:  () => t('devcard_vp_desc')      || '+1 Victory Point (kept secret)',
  roadBuilding:  () => t('devcard_road_desc')    || 'Place 2 free roads',
  yearOfPlenty:  () => t('devcard_yop_desc')     || 'Take any 2 resources from the bank',
  monopoly:      () => t('devcard_mono_desc')    || 'Claim all of one resource from everyone',
};
const DEV_CARD_EMOJI = {
  knight:'⚔️', victoryPoint:'⭐', roadBuilding:'🛤', yearOfPlenty:'🌻', monopoly:'👑',
};
...
function showDevCardDrawnPopup(drawn) {
  ...
  const emoji   = DEV_CARD_EMOJI[card] || '🃏';
  const name    = DEV_NAMES[card]  || card;
  const desc    = DEV_CARD_DESC[card]?.() || '';
  ...
  document.getElementById('dev-drawn-emoji').textContent  = emoji;
