...
class CatanGame {
  ...
  _generateBoard() {
    ...
    const ports = this._generatePorts(hexes, layout, vertices);
  ...
  _generatePorts(hexes, layout, vertices) {
    ...
    // The 9 border edges in clockwise order (pairs of outer vertex IDs)
    // Each pair is an outer edge shared by exactly 1 hex
    const BORDER_EDGES = [
      [4,  5],   // top-left
      [5,  9],   // top
      [9,  13],  // top-right
      [13, 22],  // right-upper (angle vertex)
      [22, 35],  // right
      [35, 36],  // right-lower
      [36, 45],  // lower-right angle
      [45, 52],  // bottom-right
      [52, 53],  // bottom-right edge
      [53, 51],  // bottom-right to bottom
      [51, 48],  // bottom
      [48, 49],  // bottom-left
      [49, 40],  // lower-left angle
      [40, 27],  // left-lower
      [27, 28],  // left
      [28, 17],  // left-upper
      [17, 4],   // top-left angle
    ...
    // Identify which edges are valid (both vertices outer, adjacent in the graph)
    const outerVids = new Set(
      vertices.flatMap((v,i) => v.adjHexes.length <= 2 ? [i] : [])
    );

    // Find actual border edge pairs (both outer, distance = 1 hex side)
    const borderPairs = [];
    const seen = new Set();
    for (const [a, b] of BORDER_EDGES) {
      if (a < vertices.length && b < vertices.length &&
          outerVids.has(a) && outerVids.has(b)) {
        const key = `${Math.min(a,b)}-${Math.max(a,b)}`;
        if (!seen.has(key)) { seen.add(key); borderPairs.push([a,b]); }
      }
    }
    ...
    // Pick 9 edges spaced around the perimeter
  ...
  _generateDevDeck() {
    ...
    if (this.debugDevCard) {
      const mi = deck.indexOf(this.debugDevCard);
  ...
  buyDevCard() {
    ...
    player.devCards.push({ type: card, new: true }); // new = can't play this turn
    ...
    this.lastDrawnCard = { playerId: player.id, card }; // cleared on next action
    return { ok: true, card };
  }
